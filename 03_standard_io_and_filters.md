...
1. [標準入出力](#3-1-標準入出力)
2. [リダイレクト](#3-2-1-リダイレクト標準出力の書き込み)
   - 標準出力の書き込み
   - 標準出力の追加リダイレクト（アペンド）
   - cat コマンドによるファイル作成
3. [標準エラー出力](#3-3-1-標準エラー出力)
   - 標準出力と標準エラー出力のまとめリダイレクト
   - 標準入力へのリダイレクト
4. [パイプ](#3-4-パイプ)
   - less コマンドによるページング
5. [データの先頭や末尾の表示（head / tail）](#3-5-データの先頭や末尾の表示-head--tail)
6. [テキストファイルのソート（sort コマンド）](#3-6-テキストファイルのソート-sort-コマンド)
   - 単純なソート
   - 逆順のソート
   - 列を指定したソート
   - 数値としてのソート
7. [行の重複の消去（uniq コマンド）](#3-7-行の重複の消去-uniq-コマンド)
   - データ作成例
   - 重複行の消去
8. [文字をカウントする（wc コマンド）](#3-8-文字をカウントする-wc-コマンド)
   - 文字のカウント
9. [文字列を検索する（grep コマンド）](#3-9-grep-コマンドによる文字列検索)
   - 正規表現の利用
   - grep コマンドによる文字列検索
     - 単純な文字列検索
     - 行頭の文字列指定
...
◀ [2 章へ](02_basic_commands.md) | 📚 [全体の目次](README.md) | [4 章へ](04_vi_editor.md) ▶
