...
本章では Linux システムにおける **ユーザー管理とグループ管理** を学習します。  
ユーザーやグループを適切に管理することで、システムの安全性を確保し、アクセス権限を効率的に制御できます。
...
- ユーザーの作成・変更・削除方法
- グループの作成・変更・削除方法
- `/etc/passwd`、`/etc/shadow`、`/etc/group` の構造と運用

---

## 目次

- [5-1 ユーザーとグループ](#5-1-ユーザーとグループ)
- [5-2 システムを管理するユーザー root](#5-2-システムを管理するユーザー-root)
  - [5-2-1 su コマンド](#5-2-1-su-コマンド)
  - [5-2-2 sudo コマンド](#5-2-2-sudo-コマンド)
  - [5-2-3 sudo コマンドを使って root 権限でコマンドを実行する](#5-2-3-sudo-コマンドを使って-root-権限でコマンドを実行する)
  - [5-2-4 wheel グループ所属を確認する](#5-2-4-wheel-グループ所属を確認する)
  - [5-2-5 sudo コマンドをパスワード無しで実行できるようにする](#5-2-5-sudo-コマンドをパスワード無しで実行できるようにする)
- [5-3 ユーザーの管理](#5-3-ユーザーの管理)
  - [5-3-1 ユーザーの作成](#5-3-1-ユーザーの作成)
  - [5-3-2 パスワードの設定](#5-3-2-パスワードの設定)
  - [5-3-3 root のパスワードを設定して su コマンドを実行する](#5-3-3-root-のパスワードを設定して-su-コマンドを実行する)
  - [5-3-4 ユーザーアカウントの変更](#5-3-4-ユーザーアカウントの変更)
  - [5-3-5 ユーザーの削除](#5-3-5-ユーザーの削除)
- [5-4 グループの管理](#5-4-グループの管理)
  - [5-4-1 グループの作成](#5-4-1-グループの作成)
  - [5-4-2 グループの削除](#5-4-2-グループの削除)
- [5-5 パスワードファイルとシャドウファイル](#5-5-パスワードファイルとシャドウファイル)
  - [5-5-1 /etc/passwd（パスワードファイル）](#5-5-1-etcpasswdパスワードファイル)
  - [5-5-2 /etc/shadow（シャドウファイル）](#5-5-2-etcshadowシャドウファイル)
  - [5-5-3 要約： グループファイル (/etc/group)](#5-5-3-要約-グループファイル-etcgroup)
...
- **補足:**
  現在ではセキュリティ保護の観点から、root のパスワードを設定せず、su コマンドを実行できないように設定することが多くなっています。
...
◀ [4 章へ](04_vi_editor.md) | 📚 [全体の目次](README.md#各章リンク) | [6 章へ](06_file_ownership_and_groups.md) ▶
