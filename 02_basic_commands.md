...
- [2-1 ファイルとディレクトリの参照](#2-1-ファイルとディレクトリの参照)
- [2-2 ls コマンドによるファイルやディレクトリの参照](#2-2-ls-コマンドによるファイルやディレクトリの参照)
- [2-3 コマンドの構成要素](#2-3-コマンドの構成要素)
- [2-4 ls の応用](#2-4-ls-の応用)
- [2-5 オプションの組み合わせとワイルドカード](#2-5-オプションの組み合わせとワイルドカード)
- [2-6 ファイル内容の表示](#2-6-ファイル内容の表示)
  - [2-6-1 ページャを使ったファイル内容の表示（less コマンド）](#2-6-1-ページャを使ったファイル内容の表示less-コマンド)
- [2-7 空のファイルを作成する（touch コマンド）](#2-7-空のファイルを作成するtouch-コマンド)
- [2-8 ディレクトリの操作](#2-8-ディレクトリの操作)
  - [2-8-1 カレントディレクトリの表示（pwd コマンド）](#2-8-1-カレントディレクトリの表示pwd-コマンド)
  - [2-8-2 ディレクトリの変更（cd コマンド）](#2-8-2-ディレクトリの変更cd-コマンド)
  - [2-8-3 ディレクトリの作成（mkdir コマンド）](#2-8-3-ディレクトリの作成mkdir-コマンド)
  - [2-8-4 ディレクトリの削除（rmdir コマンド）](#2-8-4-ディレクトリの削除rmdir-コマンド)
  - [2-8-5 rm -r コマンドによるディレクトリとファイルの一括削除](#2-8-5-rm--r-コマンドによるディレクトリとファイルの一括削除)
- [2-9 特別なディレクトリとパスの指定](#2-9-特別なディレクトリとパスの指定)
  - [2-9-1 特別なディレクトリ](#2-9-1-特別なディレクトリ)
  - [2-9-2 パスの絶対指定と相対指定](#2-9-2-パスの絶対指定と相対指定)
  - [2-9-3 例](#2-9-3-例)
- [2-10 ファイルのコピー（cp コマンド）](#2-10-ファイルのコピーcp-コマンド)
  - [2-10-1 基本書式](#2-10-1-基本書式)
  - [2-10-2 ファイルをディレクトリにコピー](#2-10-2-ファイルをディレクトリにコピー)
  - [2-10-3 ファイルを別名でコピー](#2-10-3-ファイルを別名でコピー)
  - [2-10-4 ファイルの上書きコピー](#2-10-4-ファイルの上書きコピー)
  - [2-10-5 ディレクトリのコピー](#2-10-5-ディレクトリのコピー)
- [2-11 ファイルとディレクトリの移動（mv コマンド）](#2-11-ファイルとディレクトリの移動mv-コマンド)
  - [2-11-1 ファイルを別のディレクトリに移動](#2-11-1-ファイルを別のディレクトリに移動)
  - [2-11-2 ディレクトリを別のディレクトリに移動](#2-11-2-ディレクトリを別のディレクトリに移動)
  - [2-11-3 ファイル名の変更](#2-11-3-ファイル名の変更)
  - [2-11-4 ディレクトリ名の変更](#2-11-4-ディレクトリ名の変更)
- [2-12 ファイルの削除（rm コマンド）](#2-12-ファイルの削除rm-コマンド)
- [2-13 削除したファイルは復元できない](#2-13-削除したファイルは復元できない)
- [2-14 ファイルの検索（find コマンド）](#2-14-ファイルの検索find-コマンド)
- [2-15 コマンドのパスを表示（which コマンド）](#2-15-コマンドのパスを表示which-コマンド)
  - [2-15-1 環境変数 PATH の設定確認](#2-15-1-環境変数-path-の設定確認)
- [2-16 コマンドのヘルプを表示する](#2-16-コマンドのヘルプを表示する)
- [2-17 マニュアルの使い方](#2-17-マニュアルの使い方)
  - [2-17-1 マニュアルのセクション](#2-17-1-マニュアルのセクション)
  - [2-17-2 セクションを指定したマニュアルの表示](#2-17-2-セクションを指定したマニュアルの表示)
...
- **サブコマンド**  
  ...
  <image src="images/サブコマンド.png" alt="サブコマンド" width="600">
...
<image src="images/ドットファイル.png" alt="ドットファイル" width="600">
